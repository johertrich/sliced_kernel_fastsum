...
dtype=torch.float
standard_chi_squared=torch.distributions.chi2.Chi2(torch.tensor([1.0]))
gamma_dist=torch.distributions.gamma.Gamma(torch.tensor([.5]),torch.tensor([.5]))

def sample_multivariate_standard_Cauchy(n,d):
    gamma_rands=torch.sqrt(standard_chi_squared.sample((n,))).to(device)
    out=torch.randn((n,d),dtype=dtype,device=device)/gamma_rands
    return out    

def RFF_Lap(x,y,x_weights,alpha,P):
    d=x.shape[1]
    tic=time.time()
    xi=sample_multivariate_standard_Cauchy(P,d)
    xi=xi.unsqueeze(1)
    b=2*math.pi*torch.rand((P,),dtype=dtype,device=device)
    x_proj=torch.nn.functional.conv1d(x.reshape(1,1,-1),xi,stride=d).reshape(P,-1).transpose(0,1)*alpha
    y_proj=torch.nn.functional.conv1d(y.reshape(1,1,-1),xi,stride=d).reshape(P,-1).transpose(0,1)*alpha
...
def RFF_Lap_batched(x,y,x_weights,alpha,P,batch_size):
    # batching over the projections
    P_left=P
    out=0
    while P_left>batch_size:
        out=out+RFF_Lap(x,y,x_weights,alpha,batch_size)/P*batch_size
        P_left-=batch_size
    out=out+RFF_Lap(x,y,x_weights,alpha,P_left)/P*P_left
    return out
    ...
    for _ in range(runs):
        ...
        for P_num,P in enumerate(P_list):
            ...
            for _ in range(multiplicities):
                out_fastsum=fast_Laplacian_summation_batched(x,y,x_weights,alpha,P,n_ft,sliced_factor,x_range=0.2,batch_size=1000)
            toc=time.time()-tic
        ...
        for P_num,P in enumerate(P_list):
            ...
            tic=time.time()
            for _ in range(multiplicities):
                out_RFF=RFF_Lap_batched(x,y,x_weights,alpha,100*P,batch_size=10000)
            toc=time.time()-tic
...
res=kernel_sum_comparison(10000,d,.05,[10,1000,2000,3000,4000],runs=1,multiplicities=1,n_ft=2048)
...
for d in ds:
    sliced_factor=compute_sliced_factor(d)
    res=kernel_sum_comparison(20000,d,.05,[10,10,50,100,200,500,1000,2000,5000,10000],runs=10,multiplicities=1,n_ft=2048)
    time_naive_mean,time_naive_std,time_fastsum_mean,time_fastsum_std,error_fastsum_mean,error_fastsum_std,time_RFF_mean,time_RFF_std,error_RFF_mean,error_RFF_std=res
    print(error_RFF_mean,error_fastsum_mean)
    print(time_RFF_mean,time_fastsum_mean)
    print(time_naive_mean)

    scipy.io.savemat(f'RFF_vs_fastsum_Lap_{d}.mat',{'runtimes_RFF':np.array(time_RFF_mean)[1:],'errors_RFF':np.array(error_RFF_mean)[1:],'runtimes_fastsum':np.array(time_fastsum_mean)[1:],'errors_fastsum':np.array(error_fastsum_mean)[1:]})
