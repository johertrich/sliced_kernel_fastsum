...
import torch
from fastsum import naive_kernel_sum, fast_Matern_summation_batched
import math
import numpy as np
...
def kernel_sum_comparison(N,d,alpha,P_list,nu,runs=10,multiplicities=10,n_ft=200):
    torch.cuda.empty_cache()
    errors_fastsum=[[] for P in P_list]
    times_naive=[]
    times_fastsum=[[] for P in P_list]
    for _ in range(runs):
        kernel_mat=lambda x,y: Matern32_kernel_mat(x,y,alpha)
        ...
        for P_num,P in enumerate(P_list):
            ...
            tic=time.time()
            for _ in range(multiplicities):
                out_fastsum=fast_Matern_summation_batched(x,y,x_weights,alpha,P,n_ft,nu,x_range=0.2,batch_size=1000)
            toc=time.time()-tic
...
if tab1:
    ...
    for N in Ns:
        ...
        results.append(kernel_sum_comparison(N,d,alpha,Ps,nu,runs=10,multiplicities=1,n_ft=n_ft))
    print(results)
    ...
    print(runtimes)
    with open('Matern_table_runtimes','w') as f:
        f.writelines(lines)
    with open('Matern_table_errors','w') as f:
        f.writelines(lines_err)
    scipy.io.savemat('Matern_results.mat',{'runtimes':runtimes,'errors':errors,'Ns':Ns,'Ps':Ps,'d':d})
...
if tab2:
    ...
    errors=np.array(errors)
    print(runtimes)
    with open('Matern_table_runtimes_base_d','w') as f:
        f.writelines(lines)
    with open('Matern_table_errors_base_d','w') as f:
        f.writelines(lines_err)
    scipy.io.savemat('Matern_results_d.mat',{'runtimes':runtimes,'errors':errors,'N':N,'Ps':Ps,'ds':ds})
...
if tab3:
    ...
    errors=np.array(errors)
    print(runtimes)
    with open('Matern_table_runtimes_base_P','w') as f:
        f.writelines(lines)
    with open('Matern_table_errors_base_P','w') as f:
        f.writelines(lines_err)
    scipy.io.savemat('Matern_results_P.mat',{'runtimes':runtimes,'errors':errors,'N':N,'Ps':Ps,'ds':ds})
