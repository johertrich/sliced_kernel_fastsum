...
def kernel_sum_comparison(N,d,P_list,runs=10,multiplicities=10):
    ...
    for _ in range(runs):
        ...
        for _ in range(multiplicities):
            out_naive=naive_kernel_sum_keops_Gauss(x,x_weights,y,sigma_sq,batch_size=1000)
        torch.cuda.synchronize()
        toc=time.time()-tic
        ...
        for P_num,P in enumerate(P_list):
            ...
            tic=time.time()
            for _ in range(multiplicities):
                out_fastsum=fast_Gaussian_summation_batched(x,y,x_weights,sigma_sq,P,1000,x_range=0.2,batch_size=int(1e8)//N)
            torch.cuda.synchronize()
            toc=time.time()-tic
...
scipy.io.savemat('Gauss_vs_keops.mat',{'times_naive':times_naive,'times_fastsum':times_fastsum,'Ns':Ns,'Ps':Ps,'d':d})
